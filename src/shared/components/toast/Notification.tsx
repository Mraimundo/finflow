...
import type { ToastType } from "../../lib/toast-event";
...
export const Notification: React.FC<NotificationProps> = React.memo(
  ({ type, title, message, showIcon = true, duration, onClose }) => {
      ...
      >
        ...
        <div className="relative z-10 flex items-center space-x-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-normal text-gray-900 dark:text-gray-100 text-lg truncate">
              {title}
            </p>
            {message && (
              <p className="text-sm text-gray-800 dark:text-gray-300 mt-1 line-clamp-2">
                {message}
              </p>
            )}
          </div>
          ...
          <button
            onClick={onClose}
            className="shrink-0 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-50"
